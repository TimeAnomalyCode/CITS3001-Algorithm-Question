...
\begin{flushleft}
    A graph word(\textbf{GW}) is a word that starts from the source node and ends at the leaf node. Example, \code{ACT+}, \code{APZ} and \code{APPLE.} is a graph word. \code{APPA} or \code{ACT} are not graph words. \\
\end{flushleft}
